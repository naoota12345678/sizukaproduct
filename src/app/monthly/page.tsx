...
import { getMonthlyProductions, aggregateByProduct, type ProductionSummary } from "@/lib/firebase";
...
export default function MonthlyPage() {
  ...
  const handleSearch = async () => {
    ...
    setSearched(true);
    try {
      const prods = await getMonthlyProductions(year, month);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-800 mb-6">月次表示</h1>

      {/* 年月選択 */}
      <div className="bg-white rounded-lg shadow border border-slate-200 p-4 mb-6 flex items-center gap-4">
        <label className="text-sm font-medium text-slate-700">年月:</label>
        <select
          value={year}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 text-left text-sm text-slate-600">
                  ...
                  <th className="px-6 py-3 font-medium text-right">合計数量</th>
                </tr>
              </thead>
